...
import {
  ...
  AlertCircle, TrendingUp, CheckCircle2, Loader2,
} from 'lucide-react'
...
const CRITERIOS_HAB = [
  ...
  { key: 'aet',          label: 'Carga Indivisível — Autorização Especial de Trânsito (AET)', desc: 'Experiência em transporte de cargas que excedem dimensões máximas regulamentares' },
  { key: 'coletivo',     label: 'Transporte Coletivo de Passageiros', desc: 'Curso adicional obrigatório para condutores de veículos de passageiros' },
  ...
  { key: 'nr11',         label: 'NR-11 — Movimentação de Cargas', desc: 'Norma regulamentadora para operações de transporte e manuseio de materiais' },
]
...
const DEFAULT_FORM: FormState = {
  ...
  criterios_op: [], beneficios: ['Adiantamento', 'Seguro de carga', 'Rastreamento'],
...
export default function VagaForm({ mode, vagaId }: VagaFormProps) {
  ...
  // ── Load existing vaga for edit mode ────────────────────────────────────────
  useEffect(() => {
    ...
    async function loadVaga() {
      ...
      setForm({
        titulo: vaga.titulo ?? '',
        rota_origem: vaga.rota_origem ?? '',
        uf_origem: vaga.uf_origem ?? '',
        rota_destino: vaga.rota_destino ?? '',
        uf_destino: vaga.uf_destino ?? '',
        km_estimado: vaga.km_estimado != null ? String(vaga.km_estimado) : '',
        frequencia_tipo: vaga.frequencia_tipo ?? '',
        tipo_veiculo: vaga.tipo_veiculo ?? '',
        tipo_equipamento: vaga.tipo_equipamento ?? '',
        equip_obs: vaga.equip_obs ?? '',
        vagas_abertas: vaga.vagas_abertas != null ? String(vaga.vagas_abertas) : '1',
        inicio_previsto: vaga.inicio_previsto ?? 'Imediato',
        tipo_carga: vaga.tipo_carga ?? 'Carga Seca',
        ano_maximo_veiculo: vaga.ano_maximo_veiculo ?? 2020,
        valor_km: vaga.valor_km != null ? String(vaga.valor_km) : '',
        forma_pagamento: vaga.forma_pagamento ?? 'Semanal',
        adiantamento: vaga.adiantamento != null ? String(vaga.adiantamento) : '',
        criterios_hab: vaga.criterios_hab ?? ['cnh_e'],
        criterios_doc: vaga.criterios_doc ?? ['rntrc', 'tac'],
        criterios_op: vaga.criterios_op ?? [],
        beneficios: vaga.beneficios ?? [],
        periodo_meses: vaga.periodo_meses != null ? String(vaga.periodo_meses) : '',
        jornada: vaga.jornada ?? '',
        descricao: vaga.descricao ?? '',
      })
      setCustomReqs(vaga.requisitos_adicionais ?? [])
  ...
  function selectImpl(name: string) {
    setForm(prev => ({ ...prev, tipo_equipamento: prev.tipo_equipamento === name ? '' : name }))
  }

  // ── Toggles ──────────────────────────────────────────────────────────────────
  function toggleCriterio(key: string, field: 'criterios_hab' | 'criterios_doc' | 'criterios_op') {
    setForm(prev => ({
      ...prev,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setSubmitError(msg ?? `Erro ao ${mode === 'create' ? 'publicar' : 'salvar'} vaga. Tente novamente.`)
    } finally {
      setLoading(false)
    }
  }

  // ── Fetch loading (edit mode) ─────────────────────────────────────────────
  if (fetchLoading) {
    return (
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-20 px-4">
        ...
        <div className="flex gap-3 justify-center">
          ...
          <Button variant="secondary" onClick={() => { setForm(DEFAULT_FORM); setCustomReqs([]); setSuccess(false) }}>
            Publicar outra
          </Button>
        </div>
      </div>
  ...
  const backHref = isEdit ? `/transportadora/vagas/${vagaId}` : '/transportadora/vagas'
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="font-serif text-2xl font-bold text-text-primary">
            {isEdit ? 'Editar vaga' : 'Publicar vaga de agregamento'}
          </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-surface border border-border rounded-xl p-5 space-y-4">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-2.5 rounded-lg border text-left transition-all ${
                      form.tipo_equipamento === im.name
                        ...
                        : im.empresa && !form.tipo_equipamento
          ...
          {form.tipo_equipamento && (
            <Input
              label="Observações sobre o equipamento (opcional)"
        ...
        <div className="bg-surface border border-border rounded-xl p-5 space-y-3">
          ...
          {CRITERIOS_HAB.map(c => (
            <CriterioItem
              ...
              onToggle={() => toggleCriterio(c.key, 'criterios_hab')}
            />
          ))}
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-5 space-y-3">
          ...
          {CRITERIOS_DOC.map(c => (
            <CriterioItem
              ...
              onToggle={() => toggleCriterio(c.key, 'criterios_doc')}
            />
          ))}
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-5 space-y-3">
          ...
          {CRITERIOS_OP.map(c => (
            <CriterioItem
              ...
              onToggle={() => toggleCriterio(c.key, 'criterios_op')}
            />
          ))}
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-5 space-y-3">
          <div className="flex items-center gap-2 mb-1">
            <FileText size={16} className="text-text-muted" />
            <h2 className="font-semibold text-text-primary text-sm uppercase tracking-wide">Requisitos adicionais (livre)</h2>
          </div>
          <p className="text-xs text-text-secondary">Adicione qualquer exigência específica não contemplada acima.</p>
